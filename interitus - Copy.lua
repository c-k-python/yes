...
pui.accent = "00FFCCFF" -- lua main color
pui.macros.heart = '❤'
...
local _ui = {
    lua = {
        enable_lua = group:checkbox("\vRemn\rant".." | \v".. euphemia.username),
    ...
    visuals = {
        ...
        watermarks = group:combobox("Watermark", {"Off", "Default", "Minimal", "Legacy", "remnant", "Country-Based"}),
...
for i, state in ipairs(aa_table.defensive_states) do 
    ...
    menu.pitch = group:combobox("Pitch", 'Disabled', 'Up', 'Zero', 'Random', 'remnant', 'Custom')
    menu.custom_pitch = group:slider("\n ", -89, 89, 0, true, "°")
...
local ctx = (function()
    ...
    ctx.antiaim = {
        run = function(cmd)
            ...
            local pitch_tbl = {
                ...
                ['remnant'] = aguwno > 2 and aguwno < 14 and -89 or 89,
        ...
        render = function()
            ...
            if _ui.visuals.inds_style.value == "Pixel" then
                if funcs.contains(_ui.visuals.inds_options.value, "Animated") then
                    renderer.text(X /2 + (vector(renderer.measure_text("c-", "remnant  " .. euphemia.build)).x /2 + 5) * fraction, y + 8, r, g, b, 255 * fraction_alpha, "c-", 0, funcs.text_fade_animation(5, r, g, b, 255 * fraction_alpha, string.upper("remnant  " .. euphemia.build)))
                    ...
                    renderer.text(X /2 + (vector(renderer.measure_text("c-", "remnant  " .. euphemia.build)).x /2 + 5) * fraction, y + 8, r, g, b, 255 * fraction_alpha, "c-", 0, string.upper("remnant  " .. euphemia.build))
                    renderer.text(X /2 + (vector(renderer.measure_text("c-", string.upper(ctx.helps.get_state_defensive(ctx.helps.speed())))).x /2 + 2) * fraction, y + 16, r, g, b, 255 * fraction_alpha, "c-", 0, string.upper(ctx.helps.get_state_defensive(ctx.helps.speed())))
                    local m_indicators = {{text = "DT", color = is_dt == true and {r,g,b} or {92,92,92}},{text = "OS",color = is_hs == true and {r,g,b} or {92,92,92}}, {text = "QP", color = is_qp == true and {r,g,b} or {92,92,92}}, {text = "FD", color = is_fd == true and {r,g,b} or {92,92,92}}}
                    for i, v in pairs(m_indicators) do
                        r,g,b = unpack(v.color)
            ...
            if _ui.visuals.inds_style.value == "Minimal" then
                if funcs.contains(_ui.visuals.inds_options.value, "Animated") then
                    renderer.text(X /2 + (vector(renderer.measure_text("c-", "remnant")).x /2 + 7) * fraction, y + 8, r, g, b, 255 * fraction_alpha, "c", 0, funcs.text_fade_animation(1.5, r,g,b,a * fraction_alpha, "remnant"))
                    ...
                    renderer.text(X /2 + (vector(renderer.measure_text("c-", "remnant")).x /2 + 7) * fraction, y + 8, r, g, b, 255 * fraction_alpha, "c", 0, "remnant")
            ...
            if _ui.visuals.inds_style.value == "Modern" then
                if funcs.contains(_ui.visuals.inds_options.value, "Animated") then
                    renderer.text(X /2 + (vector(renderer.measure_text("c-", "remnant")).x /2 + 7) * fraction, y + 8, r, g, b, 255 * fraction_alpha, "c", 0, to_jitter and "\a"..funcs.rgba_to_hex(r,g,b,255).."inte\aFFFFFFFFritus" or "\aFFFFFFFFinte\a"..funcs.rgba_to_hex(r,g,b,255).."ritus")
                    renderer.text(X /2 + (vector(renderer.measure_text("c-", euphemia.build)).x /2 + 6) * fraction, y + 18, r, g, b, 255* fraction_alpha, "c", 0, funcs.text_fade_animation(1.5, r,g,b,a * fraction_alpha, euphemia.build))
                    local m_indicators = {{text = "dt", color = is_dt == true and {r,g,b} or {92,92,92}},{text = "os",color = is_hs == true and {r,g,b} or {92,92,92}}, {text = "qp", color = is_qp == true and {r,g,b} or {92,92,92}}, {text = "fd", color = is_fd == true and {r,g,b} or {92,92,92}}}
                    for i, v in pairs(m_indicators) do
                        r,g,b = unpack(v.color)
    ...
    ctx.watermark = {
        ...
        render = function()
            ...
            local final_text = to_jitter and funcs.gradient_text(r,g,b, 255* fraction, r,g,b, 55, "remnant.RED") or funcs.gradient_text(r,g,b, 55, r,g,b, 255* fraction, "remnant.RED")
            ...
            elseif _ui.visuals.watermarks.value == "Off" then
                ...
                renderer.text(X/2, Y - 10, r, g, b, 255, "c", 0, "remnant.red")
            elseif _ui.visuals.watermarks.value == "Legacy" then
                renderer.text(pos == "Bottom" and x + 1 or x + 1, y + 1, 0,0,0,255* fraction, "cb", 0, "remnant.RED")
                renderer.text(pos == "Bottom" and x + 1 or x + 1, y - 1, 0,0,0,255* fraction, "cb", 0, "remnant.RED")
                renderer.text(pos == "Bottom" and x - 1 or x - 1, y - 1, 0,0,0,255* fraction, "cb", 0, "remnant.RED")
                renderer.text(pos == "Bottom" and x - 1 or x - 1, y + 1, 0,0,0,255* fraction, "cb", 0, "remnant.RED")
                renderer.text(pos == "Bottom" and x or x, y, 255, 255, 255, 255* fraction, "cb", nil, final_text)
            elseif _ui.visuals.watermarks.value == "remnant" then
                renderer.text(pos == "Bottom" and x or x, y, 255, 255, 255, 255* fraction, "cb", nil, final_text)
            elseif _ui.visuals.watermarks.value == "Country-Based" then
                ...
                renderer.text(pos == "Bottom" and x - 17 or 32, pos == "Bottom" and y - 11 or y - 11, 255,255,255,255* fraction, "-", 0, "remnant.RED")
    ...
    ctx.debug_panel = {
        render = function()
            ...
            if _ui.visuals.debug_panel:get() == "Modern" then
                local text = "remnant  ["..euphemia.build.."]  |  "..euphemia.username.."  |  "..ctx.watermark.round(client.latency() * 1000, 0).."ms"
            ...
            elseif _ui.visuals.debug_panel:get() == "Default" then
                ...
                renderer.text(debug_pos.pos_x + 8, debug_pos.pos_y, 255, 255, 255, 255, "", 0, "remnant - "..euphemia.username)
...
client.color_log(0, 225, 204, " remnant\0")
client.color_log(255, 255, 255, ", " .. euphemia.username)

new_notify("Welcome to \a"..funcs.rgba_to_hex(0,225,204,255).."remnant \aFFFFFFFF"..euphemia.username, 255, 115, 105,255)
...
ui_load_cfgs = ui.new_button("aa", "anti-aimbot angles", "\a00FFCCFFLoad", function() end), function() 
    return
end
ui_save_cfgs = ui.new_button("aa", "anti-aimbot angles", "\a00FFCCFFSave", function() end), function() 
    return
end
ui_delete_cfgs = ui.new_button("aa", "anti-aimbot angles", "\a00FFCCFFDelete", function() end), function() 
    return
end
ui_import_cfgs = ui.new_button("aa", "anti-aimbot angles", "\a00FFCCFFImport settings", function() end)
ui_export_cfgs = ui.new_button("aa", "anti-aimbot angles", "\a00FFCCFFExport settings", function() end)
